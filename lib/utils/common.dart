...
import 'package:handyman_provider_flutter/models/LatLong.dart';
...
void setOneSignal([BuildContext? context]) async {
  ...
  print('before observer in provider');
  OneSignal.shared.setSubscriptionObserver((changes) async {
    ...
    print('observer of user app${changes.to.userId}');
    print('user ---> getStringAsync(PLAYERID) ${getStringAsync(PLAYERID)}');
    updatePlayerId(playerId: changes.to.userId.validate());
  });

  updatePlayerId(playerId: (await OneSignal.shared.getDeviceState())!.userId.validate());
}
//endregion
